	...
	export default{
		...
		data(){
			return {
				...
				selectedOptions: [],
				options: ["History & Biography", "Fiction", "Science & Technology", "Children & Young Adult", "Business & Economics", "Philosophy & Religion", "Art & Photography"]
			}
		},
		methods:{
			changeType() {
			...
			login() {
				...
				if (self.loginEmail != "" && self.loginPassword != "") {
					// localStorage.clear();
					// localStorage.setItem("UserInfo",self.selectedOptions);
					this.$emit('getUserInfo',localStorage.getItem("UserInfo"));
					...
					// 		selectedList:this.selectedOptions
					// 	}
					// })
					// .then( res => {
			...
			register(){
				...
				if(self.registerAccount != "" && self.registerPassword != "" && self.registerCheckPassword != ""){
					...
					// 		selectedList:self.selectedOptions
					// 	}
					// })
					// .then( res => {
					// 	if(res.data.data>0){
